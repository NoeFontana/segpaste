...
def check_panoptic_thing_stuff_consistent(
    sample: DenseSample, schema: PanopticSchema
) -> InvariantReport:
    """``z(p) == 0`` iff ``s(p)`` is a stuff class."""
    name = "panoptic.thing_stuff_consistent"
    ...
    if bool(mismatch.any()):
        ...
        return InvariantReport(
            name=name,
            ok=False,
            message=(f"{violator_count} pixel(s) violate z(p)==0 <=> s(p) is stuff"),
...
def assert_panoptic_thing_stuff_consistent(
    ...
    raise_if_violated(check_panoptic_thing_stuff_consistent(sample, schema))


def check_panoptic_pixel_bijection(sample: DenseSample) -> InvariantReport:
    """On thing pixels, exactly one instance mask is set per pixel."""
    name = "panoptic.pixel_bijection"
    ...
    if bool(violators.any()):
        violator_count = int(violators.sum().item())
        return InvariantReport(
            name=name,
            ok=False,
            message=(f"{violator_count} thing pixel(s) violate sum_i 1[M_i(p)==1]==1"),
            details={"violator_count": violator_count},
        )
    return InvariantReport(name=name, ok=True)
...
def assert_panoptic_pixel_bijection(sample: DenseSample) -> None:
    raise_if_violated(check_panoptic_pixel_bijection(sample))


def check_panoptic_fresh_instance_ids(
    before: DenseSample, after: DenseSample
) -> InvariantReport:
    """Pasted instances get ids strictly greater than ``max_j z_j^b``."""
    name = "panoptic.fresh_instance_ids"
    ...
    for nid in new_ids:
        if nid <= before_max:
            return InvariantReport(
                name=name,
                ok=False,
                message=f"new instance id {nid} is <= max pre-paste id {before_max}",
                details={"new_id": int(nid), "before_max": before_max},
            )
    return InvariantReport(name=name, ok=True)


def assert_panoptic_fresh_instance_ids(before: DenseSample, after: DenseSample) -> None:
    raise_if_violated(check_panoptic_fresh_instance_ids(before, after))


def check_panoptic_stuff_area_threshold(
    before: DenseSample,
    after: DenseSample,
    ...
    name = "panoptic.stuff_area_threshold"
...
def assert_panoptic_stuff_area_threshold(
    ...
    raise_if_violated(
        check_panoptic_stuff_area_threshold(before, after, schema, tau_stuff)
    )
