...
Discriminated union over source-selection strategies; ``kind`` is the
discriminator so YAML/JSON configs round-trip cleanly through
``model_validate`` / ``model_dump``. :func:`build_source_strategy` turns
a config into a live :class:`SourceStrategy` instance, threading in the
...
class BankSourceConfig(BaseModel):
    """External instance-bank source (ADR-0011).

    The bank itself (a backend opened on disk) is constructed
