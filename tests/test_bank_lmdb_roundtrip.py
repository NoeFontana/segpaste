"""Round-trip tests for :class:`LMDBBank` (ADR-0011 PR5)."""
...
from segpaste._internal.bank.lmdb_backend import (  # noqa: E402
    LMDBBank,
    write_lmdb_bank,
)
...
def test_lmdb_bank_satisfies_protocol(tmp_path: Path) -> None:
    root, _ = _build_bank(tmp_path)
    bank: InstanceBank = LMDBBank(root)
...
def test_embeddings_round_trip(tmp_path: Path) -> None:
    root, ref = _build_bank(tmp_path, n=3, with_embeddings=True)
    bank = LMDBBank(root)
...
def test_class_frequencies_match_classes(tmp_path: Path) -> None:
    root, ref = _build_bank(tmp_path, n=12, num_classes=5)
    bank = LMDBBank(root)
...
def test_two_writes_byte_identical(tmp_path: Path) -> None:
    a, _ = _build_bank(tmp_path / "a", seed=7)
    b, _ = _build_bank(tmp_path / "b", seed=7)
    assert LMDBBank(a).version == LMDBBank(b).version
